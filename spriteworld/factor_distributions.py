...
class Gaussian(AbstractDistribution):
  ...
  def __init__(self, key, mean, stdev, dtype='float32'):
    """Construct continuous 1-dimensional uniform distribution.
  ...
  def sample(self, rng=None):
    """Sample value with mean self.mean and standard deviation self.stdev and return dict."""
    rng = self._get_rng(rng)
    out = rng.normal(loc=self.mean, scale=self.stdev)
    out = np.cast[self.dtype](out)
    return {self.key: out}
  ...
  def to_str(self, indent):
    s = '<Gaussian: key={}, mean={}, stdev={}, dtype={}>'.format(
        self.key, self.mean, self.stdev, self.dtype)
    return indent * '  ' + s
